...
function energy(ϕ,ψ,Z,dϕdt,dψdt,dZdt,ZED,meanSqrRenorm,zPE)
    ...
    #!v2 -fully uses spmd (needs to be speed tested)
    d_out= ddata();
    spmd(energy_calculation!,ϕ,ψ,Z,dϕdt,dψdt,dZdt,ZED,meanSqrRenorm,zPE,d_out; pids=workers())
    totalE = sum(d_out)
...
#!SPMD Version
function energy_calculation!(ϕ,ψ,Z,dϕdt,dψdt,dZdt,ZED,meanSqrRenorm,zPE,d_out)
    ...
    #---Using Two-Index Notation---#
        ...
        for j=padd+1:padd+Nx_loc
            for k=padd+1:padd+Ny_loc
                ...
                #Kinetic, gradient, potential energy densities
                kEϕψ = abs2(dϕdt[:L][j-padd,k-padd,1]) + (dψdt[:L][j-padd,k-padd,1])^2/2
                gEϕψ = ( ( ( (ψ[:L][j+1,k] - ψ[:L][j,k])/dx )^2  
                         + ( (ψ[:L][j,k+1] - ψ[:L][j,k])/dy )^2 )/2
                        + ( abs2( (ϕ[:L][j+1,k] - ϕ[:L][j,k])/dx ) 
                          + abs2( (ϕ[:L][j,k+1] - ϕ[:L][j,k])/dy ) ) )
                pEϕψ = -m_ϕ^2*abs2(ϕ[:L][j,k]) + m_ψ^2*ψ[:L][j,k,]^2/2 + λ*abs2(ϕ[:L][j,k])^2/2 + λ*η^4/2
                #Total classical energy density
                classicalE = kEϕψ + gEϕψ + pEϕψ
                ...
                for l=1:Nx
                    for m=1:Ny
                        kEZ = kEZ + ( abs2(dZdt[:L][j-padd,l,k-padd,m,1]) )/2
                        gEZfwd = ( abs2((Z[:L][j+1,l,k,m] - Z[:L][j,l,k,m])/dx)  
                                 + abs2((Z[:L][j,l,k+1,m] - Z[:L][j,l,k,m])/dy) )/2
                        gEZbkd = ( abs2((Z[:L][j,l,k,m] - Z[:L][j-1,l,k,m])/dx)  
                                 + abs2((Z[:L][j,l,k,m] - Z[:L][j,l,k-1,m])/dy) )/2
                        gEZ = gEZ + (gEZbkd + gEZfwd)/2
                        pEZ = pEZ + m_ρ^2* abs2(Z[:L][j,l,k,m])/2
                        intEZ = intEZ + ( α*abs2(ϕ[:L][j,k]) + β*ψ[:L][j,k]^2 ) * ( abs2(Z[:L][j,l,k,m]) )/2
                    end
                end
            
                #Renormalization
                intEZ = intEZ - ( α*abs2(ϕ[:L][j,k]) + β*ψ[:L][j,k]^2 )*meanSqrRenorm/2
                #Total energy density of Z including the interactions
                quantumEZ = (kEZ + gEZ + pEZ + intEZ)/(dx*dy)
