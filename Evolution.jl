...
#             #Move a time step
#             evolve_ulan!(ϕ_s,ψ_s,Z,dϕdt_s,dψdt_s,dZdt,meanSqrRenorm,t)
#             # Shift next time values to present time for the next step
...
#     function evolve_ulan!(ϕ,ψ,Z,dϕdt,dψdt,dZdt,meanSqrRenorm,t)
        
        
#         dt6 = dt/6
#         @views begin

#             kϕ, kψ, kZ, kϕ̇, kψ̇, kŻ = kCalculator(ϕ,ψ,Z,dϕdt,dψdt,dZdt,meanSqrRenorm)

#             #Move a time step using k's
#             #fields
#             ϕ[:,2] =   ϕ[:,1]   .+ ( kϕ[1,:] .+ 2*kϕ[2,:] .+ 2*kϕ[3,:] .+ kϕ[4,:] )./6
#             ψ[:,2] =   ψ[:,1]   .+ ( kψ[1,:] .+ 2*kψ[2,:] .+ 2*kψ[3,:] .+ kψ[4,:] )./6
#             Z[:,:,2] = Z[:,:,1] .+ ( kZ[1,:,:] .+ 2*kZ[2,:,:] .+ 2*kZ[3,:,:] .+ kZ[4,:,:] )./6

#             #dfdt's
#             dϕdt[:,2]   = dϕdt[:,1]   .+ ( kϕ̇[1,:] .+ 2*kϕ̇[2,:] .+ 2*kϕ̇[3,:] .+ kϕ̇[4,:] )./6
#             dψdt[:,2]   = dψdt[:,1]   .+ ( kψ̇[1,:] .+ 2*kψ̇[2,:] .+ 2*kψ̇[3,:] .+ kψ̇[4,:] )./6
#             dZdt[:,:,2] = dZdt[:,:,1] .+ ( kŻ[1,:,:] .+ 2*kŻ[2,:,:] .+ 2*kŻ[3,:,:] .+ kŻ[4,:,:] )./6
...
#     function kCalculator(ϕ,ψ,Z,dϕdt,dψdt,dZdt,meanSqrRenorm)

#         kϕ = im*zeros(4,N^2)
#         kϕ̇ = im*zeros(4,N^2)
#         kψ =    zeros(4,N^2)
#         kψ̇ =    zeros(4,N^2)
#         kZ = im*zeros(4,N^2,N^2)
#         kŻ = im*zeros(4,N^2,N^2)

#         #k's (1,2,3,4)
#         @views begin
#             #k1
#             A, B = dt.* fluxes_ϕ_ψ( ϕ[:,1],
#                                     ψ[:,1],
#                                     Z[:,:,1],
#                                     meanSqrRenorm)
#             kϕ̇[1,:] .= A 
#             kψ̇[1,:] .= B
#             kŻ[1,:,:] .= dt.* flux_Z(ϕ[:,1],ψ[:,1],Z[:,:,1])
#             kϕ[1,:] .= dt.* dϕdt[:,1]
#             kψ[1,:] .= dt.* dψdt[:,1]
#             kZ[1,:,:] .= dt.* dZdt[:,:,1]
...
#             #k2-k3
#             for i=2:3
#                 A, B = dt.* fluxes_ϕ_ψ( ϕ[:,1]   .+ kϕ[i-1,:]./2,
#                                         ψ[:,1]   .+ kψ[i-1,:]./2,
#                                         Z[:,:,1] .+ kZ[i-1,:,:]./2,
#                                         meanSqrRenorm)
#                 kϕ̇[i,:] .= A
#                 kψ̇[i,:] .= B
#                 kŻ[i,:,:] .= dt.* flux_Z(ϕ[:,1]   .+ kϕ[i-1,:]./2,
#                                         ψ[:,1]   .+ kψ[i-1,:]./2,
#                                         Z[:,:,1] .+ kZ[i-1,:,:]./2)
#                 kϕ[i,:] .= dt.* ( dϕdt[:,1]   .+ kϕ̇[i-1,:]./2 )
#                 kψ[i,:] .= dt.* ( dψdt[:,1]   .+ kψ̇[i-1,:]./2 )
#                 kZ[i,:,:] .= dt.* ( dZdt[:,:,1] .+ kŻ[i-1,:,:]./2 )
#             end

#             #k4
#             A, B = dt.* fluxes_ϕ_ψ( ϕ[:,1]   .+ kϕ[3,:],
#                                     ψ[:,1]   .+ kψ[3,:],
#                                     Z[:,:,1] .+ kZ[3,:,:],
#                                     meanSqrRenorm)
#             kϕ̇[4,:] .= A
#             kψ̇[4,:] .= B
#             kŻ[4,:,:] .= dt.* flux_Z(ϕ[:,1]   .+ kϕ[3,:],
#                                     ψ[:,1]   .+ kψ[3,:],
#                                     Z[:,:,1] .+ kZ[3,:,:])
#             kϕ[4,:] .= dt.* ( dϕdt[:,1]   .+ kϕ̇[3,:] )
#             kψ[4,:] .= dt.* ( dψdt[:,1]   .+ kψ̇[3,:] )
#             kZ[4,:,:] .= dt.* ( dZdt[:,:,1] .+ kŻ[3,:,:] )
#         end

#         return kϕ, kψ, kZ, kϕ̇, kψ̇, kŻ

#     end

#     function fluxes_ϕ_ψ(ϕ,ψ,Z,meanSqrRenorm)
...
#             ϕ_flux[J] = ( (ϕ[nnr_x] - 2ϕ[J] + ϕ[nnl_x])/dx^2 + (ϕ[nnr_y] - 2ϕ[J] + ϕ[nnl_y])/dy^2 
...
#             for K=1:N2
#                 Z_flux[J,K] = ( (Z[nnr_x,K] - 2Z[J,K] + Z[nnl_x,K])/dx^2 + (Z[nnr_y,K] - 2Z[J,K] + Z[nnl_y,K])/dy^2
...
# # 2-index Notation Leap-Frog Method
#         export time_evolve!
